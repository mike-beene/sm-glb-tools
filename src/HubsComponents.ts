...
const SM_NAME = "SM_components";
export class SMComponentsProperty extends ExtensionProperty {
  public readonly propertyType = "SMComponentsProperty";
  public readonly parentTypes = [PropertyType.NODE, PropertyType.SCENE, PropertyType.MATERIAL];
  public readonly extensionName = SM_NAME;
...
export class SMComponents extends Extension {
  public readonly extensionName = SM_NAME;
  public static readonly EXTENSION_NAME = SM_NAME;
  ...
  read(context: ReaderContext): this {
    ...
    const copyComponents = (defType: string) => {
      return (def, i) => {
        if (!def.extensions || !def.extensions[SM_NAME]) return;

        const prop = new SMComponentsProperty(this.doc.getGraph(), this);
        prop.data = def.extensions[SM_NAME];
        ...
        context[defType][i].setExtension(SM_NAME, prop);
      };
    };
  ...
  write(context: WriterContext) {
    this.doc
      ...
      .listNodes()
      .forEach((node) => {
        const ext = node.getExtension(SM_NAME) as SMComponentsProperty;
        if (ext) {
    ...
    this.doc
      ...
      .listScenes()
      .forEach((scene, i) => {
        const ext = scene.getExtension(SM_NAME) as SMComponentsProperty;
        if (ext) {
